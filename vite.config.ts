...
function scanRootSplashHtmlFiles(projectRoot: string): { name: string; absPath: string; urlPath: string }[] {
	...
	return entries
		...
		.map((e) => {
			...
			return { name: `splash_${baseName}`, absPath, urlPath: `/${e.name}` };
		})
		.sort((a, b) => a.urlPath.localeCompare(b.urlPath));
}

function writeSplashManifest(projectRoot: string, pages: { urlPath: string }[]): void {
	const publicDir = path.resolve(projectRoot, 'public');
	if (!fs.existsSync(publicDir)) {
		fs.mkdirSync(publicDir, { recursive: true });
	}
	const manifestPath = path.resolve(publicDir, 'splash-manifest.json');
	const payload = pages.map((p) => p.urlPath);
